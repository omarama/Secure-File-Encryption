...
void ocall_get_binary_file_size(const char *path, int pathLength, int *size)
{
	...
	if (motd) {
		int end = motd.tellg();
		memcpy(size, &end, sizeof(size));
	}
	return;
}
void ocall_get_file(char *text, int lengthText, const char *path, int pathLength)
{
...
void ocall_get_binary(char *text, int lengthText,const  char *path, int pathLength)
{
...
void ocall_write_file(char *text, int textLength, const char *path, int pathLength)
{
	if (0)
	{
...
void ocall_write_binary(char *text, int textLength, const char* path, int pathLength)
{
...
int SGX_CDECL main(int argc, char *argv[])
{
	...
	ret = ecall_AuditLoggingEnc_sample(global_eid, &ecall_ret, text, file_length, const_cast<char*>(filename.c_str()),filename.size());
	...
	ret = ecall_AuditLoggingDec_sample(global_eid, &ecall_ret, text, file_length, const_cast<char*>(filename.c_str()), filename.size());
